...
void entry(unsigned long magic, unsigned long addr) {
  ...
  unsigned int blk_start = -1;
  if (CHECK_FLAG(mbi->flags, 3)) {
    int mod_count = 0;
    int i;
    module_t *mod = (module_t *)mbi->mods_addr;
    while (mod_count < mbi->mods_count) {
      if (mod_count == 0) blk_start = (unsigned int)mod->mod_start;
  ...
  /* setup the idt with all the vectors*/
  initialize_idt();
  ...
  // execute will enable interrupts
  ...
  process_start(process_create((uint8_t *)"shell", 0, -1));
